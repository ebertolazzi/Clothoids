...
namespace Pipal
{
   ...
   */
  template <typename Real> void Solver<Real>::resetDirection( Direction<Real> & d ) const
  {
   ...
   */
  template <typename Real> void Solver<Real>::evalLinearCombination(
   ...
   * \tparam Real Floating-point type used by the algorithm.
   */
  template <typename Real> void Solver<Real>::evalModels()
  {
    // Create alias for easier access
    Input<Real> &     i{ this->m_input };
    ...
    if ( z.rho > 0 )
    {
      ...
      if ( i.nE > 0 )
      {
        ...
        d.ltred0 -= 0.5 * ( ( ( 1.0 - z.mu / z.r1 ) * ( -1.0 + z.cE / sqrt_term ) +
                              ( 1.0 - z.mu / z.r2 ) * ( 1.0 + z.cE / sqrt_term ) ) *
                            ( z.JE * d.x ).array() )
                            .sum();
      }
      if ( i.nI > 0 )
      {
        ...
        d.ltred0 -= 0.5 * ( ( ( ( -z.mu ) / z.s1 ) * ( -1.0 + z.cI / sqrt_term ) +
                              ( 1.0 - z.mu / z.s2 ) * ( 1.0 + z.cI / sqrt_term ) ) *
                            ( z.JI * d.x ).array() )
                            .sum();
      }
      ...
      if ( i.nE > 0 )
      {
        vec.segment( i.nV, 2 * i.nE ) << ( z.r1 + d.r1 ) * ( 1.0 + ( z.lE + d.lE ) ),
          ( z.r2 + d.r2 ) * ( 1.0 - ( z.lE + d.lE ) );
      }
      if ( i.nI > 0 )
      {
        vec.segment( i.nV + 2 * i.nE, 2 * i.nI ) << ( z.s1 + d.s1 ) * ( z.lI + d.lI ),
          ( z.s2 + d.s2 ) * ( 1.0 - ( z.lI + d.lI ) );
      }
   ...
   * \tparam Real Floating-point type used by the algorithm.
   */
  template <typename Real> void Solver<Real>::evalNewtonStep()
  {
    // Create alias for easier access
    Input<Real> const &   i{ this->m_input };
   ...
   * \tparam Real Floating-point type used by the algorithm.
   */
  template <typename Real> void Solver<Real>::evalStep()
  {
    // Create alias for easier access
    ...
    if ( p.algorithm == Algorithm::ADAPTIVE )
    {
      ...
      Array<Real> Mu( ( mu_curr * exponents.unaryExpr( [&p]( Real e ) { return std::pow( p.mu_factor, e ); } ) )
                        .min( p.mu_max )
                        .max( p.mu_min ) );
   ...
   */
  template <typename Real> void Solver<Real>::evalTrialStep( Direction<Real> & v ) const
  {
    Input<Real> const &     i{ this->m_input };
   ...
   * \tparam Real Floating-point type used by the algorithm.
   */
  template <typename Real> void Solver<Real>::evalTrialStepCut()
  {
    // Create alias for easier access
    Input<Real> &      i{ this->m_input };
    Direction<Real> &  d{ this->m_direction };
   ...
   */
  template <typename Real> void Solver<Real>::setDirection(
