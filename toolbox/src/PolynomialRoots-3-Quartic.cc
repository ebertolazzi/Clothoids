...
namespace PolynomialRoots
{
  ...
  integer Quartic::get_real_roots( real_type r[] ) const
  {
    integer nr = 0;
    if ( !cplx0() ) r[nr++] = m_r0;
    if ( !cplx1() ) r[nr++] = m_r1;
  ...
  integer Quartic::get_positive_roots( real_type r[] ) const
  {
    integer nr = 0;
  ...
  integer Quartic::get_negative_roots( real_type r[] ) const
  {
    integer nr = 0;
    if ( !cplx0() && m_r0 < 0 ) r[nr++] = m_r0;
    if ( !cplx1() && m_r1 < 0 ) r[nr++] = m_r1;
  ...
  integer Quartic::get_roots_in_range( real_type const a, real_type const b, real_type r[] ) const
  {
    integer nr = 0;
  ...
  integer Quartic::get_roots_in_open_range( real_type const a, real_type const b, real_type r[] ) const
  {
    integer nr = 0;
    if ( !cplx0() && m_r0 > a && m_r0 < b ) r[nr++] = m_r0;
    if ( !cplx1() && m_r1 > a && m_r1 < b ) r[nr++] = m_r1;
  ...
  static void scaleQuarticMonicPolynomial(
    real_type const A,
    real_type const B,
    real_type const C,
    real_type const D,
  ...
  static void deflateQuarticPolynomial(
    real_type const a4,
    real_type const a3,
    real_type const a2,
    real_type const a1,
  ...
  */
  ...
  static integer zeroQuarticByNewtonBisection(
    real_type const a,
    real_type const b,
    real_type const c,
    real_type const d,
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // Translate to C from Polynomial234RootSolvers
  static integer zeroHexicByNewtonBisection(
    real_type const q3,
    real_type const q2,
    real_type const q1,
    real_type const q0,
    real_type &     x )
  {
    ...
    {
      t = u - s;  // initial bisection interval
      while ( std::abs( t ) > std::abs( x ) * machepsi )
      {  // bisection iterates
        ++iter;
  ...
  */
  ...
  void Quartic::info( ostream_type & s ) const
  {
    real_type const & A{ m_ABCDE[0] };
    real_type const & B{ m_ABCDE[1] };
    real_type const & C{ m_ABCDE[2] };
  ...
  bool Quartic::check( ostream_type & s ) const
  {
    real_type const & A{ m_ABCDE[0] };
    real_type const & B{ m_ABCDE[1] };
    real_type const & C{ m_ABCDE[2] };
    ...
    real_type const & E{ m_ABCDE[4] };
    bool              ok{ true };
    real_type const epsi = 1000 * ( ( std::abs( A ) + std::abs( B ) + std::abs( C ) + std::abs( D ) + std::abs( E ) ) *
                                    machepsi );
    if ( m_ncplx > 0 )
    {
