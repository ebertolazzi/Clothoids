...
namespace Pipal
{
   ...
   */
  template <typename Real> class Output
  {
    static_assert( std::is_floating_point_v<Real>, "Pipal::Output<Real>: Real must be a floating-point type." );
  ...
  public:
    ...
    {
      this->s << std::scientific << std::setprecision( 4 ) << std::showpos << z.phi << "  " << std::noshowpos
              << z.kkt[2] << " | " << z.shift << "  " << d.x_norm << "  " << d.l_norm << "  " << std::showpos << d.ltred
              << "  " << d.qtred << "  " << d.m << std::noshowpos << " | ";
    }
    ...
    {
      this->s << std::setw( 5 ) << c.k << " | " << std::scientific << std::setprecision( 4 ) << std::showpos << z.f
    ...
    void printFooter( Counter const & c, Iterate<Real> const & z, Integer const b ) const
    {
      ...
      this->s << '\n'
              << "Final values\n"
              << "============\n"
              << std::showpos << "  Objective function........................ : " << z.fu << '\n'
