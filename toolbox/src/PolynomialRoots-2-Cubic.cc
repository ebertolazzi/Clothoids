...
namespace PolynomialRoots
{
  ...
  integer Cubic::get_real_roots( real_type r[] ) const
  {
    integer nr{ 0 };
    if ( m_cplx )
    {
  ...
  integer Cubic::get_positive_roots( real_type r[] ) const
  {
    integer nr{ 0 };
    if ( m_cplx )
    {
  ...
  integer Cubic::get_negative_roots( real_type r[] ) const
  {
    integer nr{ 0 };
    if ( m_cplx )
    {
  ...
  integer Cubic::get_roots_in_range( real_type const a, real_type const b, real_type r[] ) const
  {
    integer nr{ 0 };
    if ( m_cplx )
    {
  ...
  integer Cubic::get_roots_in_open_range( real_type const a, real_type const b, real_type r[] ) const
  {
    integer nr{ 0 };
    if ( m_cplx )
    {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  static real_type guess1( real_type const a[3] )
  {
  ...
  static real_type guess2( real_type const a[3] )
  {
    constexpr real_type p{ -1.09574 };
    constexpr real_type q{ 3.239E-1 };
    constexpr real_type r{ -3.239E-1 };
  ...
  static real_type guess3( real_type const a[3] )
  {
  ...
  static real_type guess4( real_type const a[3] )
  {
  ...
  static real_type guess5( real_type const a[3] )
  {
    real_type       p, q, r, s;
  ...
  static real_type guess6( real_type const a[3] )
  {
    real_type       p, q, r, s;
  ...
  */
  ...
  // x^3 + a * x^2 + b * x + c
  static integer NewtonBisection( real_type const a, real_type const b, real_type const c, real_type & x )
  {
    real_type p, dp;
    ...
    if ( bisection )
    {
      ...
      while ( std::abs( t ) > ( 1 + std::abs( x ) ) * machepsi && iter < MAX_ITER_SAFETY )
      {  // bisection iterates
        ++iter;
  ...
  void Cubic::info( ostream_type & s ) const
  {
    real_type const & A{ m_ABCD[0] };
    real_type const & B{ m_ABCD[1] };
    real_type const & C{ m_ABCD[2] };
  ...
  bool Cubic::check( ostream_type & s ) const
  {
    real_type const & A{ m_ABCD[0] };
    real_type const & B{ m_ABCD[1] };
    real_type const & C{ m_ABCD[2] };
    real_type const & D{ m_ABCD[3] };
    bool              ok{ true };
    real_type const   epsi{ 10 * ( std::abs( A ) + std::abs( B ) + std::abs( C ) + std::abs( D ) ) * machepsi };
    if ( m_cplx )
    {
