...
namespace PolynomialRoots
{
  ...
  //
  real_type evalPoly( real_type const op[], integer const Degree, real_type const x )
  {
    if ( std::abs( x ) > 1 )
    {
  ...
  // (op[0] + op[1]/x.... + op[n]/x^n)*x^n
  //
  bool NewtonStep( real_type const op[], integer const Degree, real_type & x )
  {
    real_type p, dp;
    if ( std::abs( x ) > 1 )
    {
  ...
  // (op[0] + op[1]/x.... + op[n]/x^n)*x^n
  //
  void evalPolyDPoly( real_type const op[], integer const Degree, real_type const x, real_type & p, real_type & dp )
  {
    if ( std::abs( x ) > 1 )
    {
  ...
  //
  // op[0] * x^n + .... + op[n-1]*x + op[n]
  complex_type evalPolyC( real_type const op[], integer const Degree, complex_type const x )
  {
    if ( std::abs( x ) > 1 )
    {
  ...
  */
  ...
  static pair<integer, real_type> scalePolynomial(
  ...
  */
    ...
    real_type       b[] )
  {
    ...
    if ( isZero( a[n] - 1 ) )
    {
      ...
      {
        b[0] = -a[0] / r;
        for ( integer j = 1; j < i_cross; ++j ) b[j] = ( a[j] - b[j - 1] ) / r;
      }
      for ( integer j{ n - 2 }; j >= i_cross; --j ) b[j] = a[j + 1] + r * b[j + 1];
    }
    else
    {
      ...
      {
        b[0] = -( a[0] / an ) / r;
        for ( integer j{ 1 }; j < i_cross; ++j ) b[j] = ( a[j] / an - b[j - 1] ) / r;
      }
      for ( integer j{ n - 2 }; j >= i_cross; --j ) b[j] = a[j + 1] / an + r * b[j + 1];
    }
  }
...
//
  ...
  //
  // x^3 + A x^2 + B x + C
  static void scaleCubicMonicPolynomial(
    real_type const A,
    real_type const B,
    real_type const C,
    real_type &     AS,
    ...
    real_type &     scale )
  {
    ...
    {
      if ( b < c )
        i_case = 0;  // a < b < c --> c MAX
      else
        i_case = 1;  // a < b and c <= b --> b MAX
    }
    else
    {
      if ( a < c )
        i_case = 0;  // b <= a < c --> c MAX
      else
        i_case = 2;  // b <= a  and c <= a --> a MAX
    }
  ...
  static void deflateCubicPolynomial(
    real_type const a3,
    real_type const a2,
    real_type const a1,
    real_type const a0,
