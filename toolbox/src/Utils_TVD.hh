...
namespace Utils
{
  ...
  //!
  //!  \brief Class for performing Total Variation Denoising (TVD) on 1D
  //!  signals.
  //!
  ...
  //!
  //!  \f[
  //!    \textrm{minimize}\quad \sum_k (y_k - x_k)^2 + \lambda \sum_k
  //!    |x_{k+1}-x_k|
  //!  \f]
  //!
  ...
  //!    | Institute of Electrical and Electronics Engineers, 2013, 20 (11),
  //!    pp.1054-1057. | DOI: <10.1109/LSP.2013.2278339>. | Available at:
  ...
  public:
    ...
    //!
    //!  This method is a convenience wrapper that calls the more detailed
    //!  `denoise` method with a default increment of 1 for both the input and
    //!  output arrays.
    //!
    //!  \param N      The number of elements in the input signal.
    //!  \param y      A pointer to the input signal array (size N).
    //!  \param lambda The regularization parameter that controls the
    //!  smoothness.
    //!  \param x      A pointer to the output denoised signal array (size N).
    //!
    static void
    denoise( Integer N, Real const y[], Real lambda, Real x[] )
    {
    ...
    //! \param N      The number of elements in the input signal.
    //! \param y      A pointer to the input signal array (size N).
    //! \param incy   The increment between consecutive elements in the input
    //! array (y).
    //! \param lambda The regularization parameter that controls the smoothness.
    //! \param x      A pointer to the output denoised signal array (size N).
    //! \param incx   The increment between consecutive elements in the output
    //! array (x).
    //!
