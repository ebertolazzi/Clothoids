...
static BaseCurve * get_base_pointer( string const & kind, mxArray const * arg )
{
...
static void do_length( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
...
#define CMD CMD_BASE "('length',OBJ[,offs,'ISO'/'SAE']): "
  ...
  {
    real_type offs;
    offs     = Utils::mex_get_scalar_value( arg_in_2, CMD "`offs` expected to be a real scalar" );
    bool ISO = true;
...
#define CMD CMD_BASE "('bbox',OBJ[,offs,'ISO'/'SAE']): "
  ...
  if ( nrhs >= 3 )
  {
    real_type offs = Utils::mex_get_scalar_value( arg_in_2, CMD "`offs` expected to be a real scalar" );
    bool      ISO  = true;
    if ( nrhs == 4 ) ISO = do_is_ISO( arg_in_3, CMD " last argument must be a string" );
    if ( ISO )
      ptr->bbox_ISO( offs, xmin, ymin, xmax, ymax );
    else
      ptr->bbox_SAE( offs, xmin, ymin, xmax, ymax );
  }
  else
  {
...
static void do_bbTriangles( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
#define CMD CMD_BASE "('bbTriangles',OBJ[,max_angle,max_size,offs,'ISO'/'SAE']): "
  ...
  real_type offs      = 0;
  if ( nrhs >= 3 ) max_angle = Utils::mex_get_scalar_value( arg_in_2, CMD "`max_angle` expected to be a real scalar" );
  if ( nrhs >= 4 ) max_size = Utils::mex_get_scalar_value( arg_in_3, CMD "`max_size` expected to be a real scalar" );
  if ( nrhs >= 5 ) offs = Utils::mex_get_scalar_value( arg_in_4, CMD "`offs` expected to be a real scalar" );
  ...
  if ( nrhs >= 5 )
  {
    if ( ISO ) { ptr->bb_triangles_ISO( offs, tvec, max_angle, max_size, 0 ); }
    else
    {
...
static void do_change_origin( int nlhs, mxArray *[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_rotate( int nlhs, mxArray *[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_reverse( int nlhs, mxArray *[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_trim( int nlhs, mxArray *[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_closest_point( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
#define CMD CMD_BASE "('closest_point',OBJ,qx,qy[,offs,'ISO'/'SAE']): "
  ...
  if ( nlhs == 1 )
  {
    ...
    static char const * fieldnames[] = { "x", "y", "s", "t", "iflag", "dst" };
  ...
  if ( nrhs >= 5 )
  {
    ...
    if ( ISO )
    {
      for ( mwSize i = 0; i < size; ++i )
        *iflag++ = ptr->closest_point_ISO( *qx++, *qy++, offs, *x++, *y++, *s++, *t++, *dst++ );
    }
    else
    {
      for ( mwSize i = 0; i < size; ++i )
        *iflag++ = ptr->closest_point_SAE( *qx++, *qy++, offs, *x++, *y++, *s++, *t++, *dst++ );
    }
  }
  else
  {
    for ( mwSize i = 0; i < size; ++i )
      *iflag++ = ptr->closest_point_ISO( *qx++, *qy++, *x++, *y++, *s++, *t++, *dst++ );
  }
...
static void do_distance( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
#define CMD CMD_BASE "('distance',OBJ,qx,qy[,offs,'ISO'/'SAE']): "
  ...
  if ( nrhs >= 5 )
  {
    ...
    if ( ISO )
    {
      for ( mwSize i = 0; i < size; ++i ) *dst++ = ptr->distance_ISO( *qx++, *qy++, offs );
    }
    else
    {
      for ( mwSize i = 0; i < size; ++i ) *dst++ = ptr->distance_SAE( *qx++, *qy++, offs );
    }
  }
  else
  {
    for ( mwSize i = 0; i < size; ++i ) *dst++ = ptr->distance( *qx++, *qy++ );
  }
...
#define CMD CMD_BASE "('collision',OBJ,OBJ1,type[,offs,offs1,'ISO'/'SAE']): "
  ...
  G2LIB_DEBUG_MESSAGE( "do_collision ADDRS: {}, {}\n", fmt::ptr( ptr0 ), fmt::ptr( ptr1 ) );

  if ( nrhs == 4 ) { Utils::mex_set_scalar_bool( arg_out_0, G2lib::collision( ptr0, ptr1 ) ); }
  else
  {
    real_type offs     = Utils::mex_get_scalar_value( arg_in_4, CMD "`offs` expected to be a real scalar" );
    ...
    bool ok;
    if ( ISO )
      ok = G2lib::collision_ISO( ptr0, offs, ptr1, offs_obj );
    else
      ok = G2lib::collision_SAE( ptr0, offs, ptr1, offs_obj );
...
#define CMD CMD_BASE "( 'intersect', OBJ, OBJ1, type, [,offs, offs1, 'ISO'/'SAE'] ): "
  ...
  G2LIB_DEBUG_MESSAGE( "do_intersect ADDRS: {}, {}\n", fmt::ptr( ptr0 ), fmt::ptr( ptr1 ) );

  IntersectList ilist;
  if ( nrhs == 4 ) { intersect( ptr0, ptr1, ilist ); }
  else
  {
    ...
    if ( nrhs == 7 ) ISO = do_is_ISO( arg_in_6, CMD " last argument must be a string" );
    if ( ISO )
      intersect_ISO( ptr0, offs, ptr1, offs_obj, ilist );
    else
      intersect_SAE( ptr0, offs, ptr1, offs_obj, ilist );
  }
...
static void do_findST( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
#define CMD CMD_BASE "('findST',OBJ,x,y[,'ISO'/'SAE']): "
  ...
  if ( ISO )
  {
    for ( mwSize i = 0; i < size; ++i ) ptr->findST_ISO( *x++, *y++, *s++, *t++ );
  }
  else
  {
    for ( mwSize i = 0; i < size; ++i ) ptr->findST_SAE( *x++, *y++, *s++, *t++ );
  }
#undef CMD
}
...
static void do_info( int nlhs, mxArray *[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
#define LOOPXY2 for ( mwSize i = 0; i < npts; ++i, s += incs, t += inct, ++pX, ++pY )
    ...
    if ( nlhs == 1 )
    {
      ...
      if ( ISO ) { LOOPXY1 ptr->eval_ISO( *s, *t, pXY[0], pXY[1] ); }
      else
      {
    ...
    else if ( nlhs == 2 )
    {
      ...
      if ( ISO ) { LOOPXY2 ptr->eval_ISO( *s, *t, *pX, *pY ); }
      else
      {
...
#define LOOPXY2 for ( mwSize i = 0; i < npts; ++i, s += incs, t += inct, ++pX, ++pY )
    ...
    {
      real_type * pXY = Utils::mex_create_matrix_value( arg_out_0, 2, size );
      if ( ISO ) { LOOPXY1 ptr->eval_ISO_D( *s, *t, pXY[0], pXY[1] ); }
      else
      {
    ...
    else if ( nlhs == 2 )
    {
      real_type * pX = Utils::mex_create_matrix_value( arg_out_0, 1, size );
      real_type * pY = Utils::mex_create_matrix_value( arg_out_1, 1, size );
      if ( ISO ) { LOOPXY2 ptr->eval_ISO_D( *s, *t, *pX, *pY ); }
      else
      {
...
#define LOOPXY2 for ( mwSize i = 0; i < npts; ++i, s += incs, t += inct, ++pX, ++pY )
    ...
    {
      real_type * pXY = Utils::mex_create_matrix_value( arg_out_0, 2, size );
      if ( ISO ) { LOOPXY1 ptr->eval_ISO_DD( *s, *t, pXY[0], pXY[1] ); }
      else
      {
    ...
    else if ( nlhs == 2 )
    {
      real_type * pX = Utils::mex_create_matrix_value( arg_out_0, 1, size );
      real_type * pY = Utils::mex_create_matrix_value( arg_out_1, 1, size );
      if ( ISO ) { LOOPXY2 ptr->eval_ISO_DD( *s, *t, *pX, *pY ); }
      else
      {
...
#define LOOPXY2 for ( mwSize i = 0; i < npts; ++i, s += incs, t += inct, ++pX, ++pY )
    ...
    {
      real_type * pXY = Utils::mex_create_matrix_value( arg_out_0, 2, size );
      if ( ISO ) { LOOPXY1 ptr->eval_ISO_DDD( *s, *t, pXY[0], pXY[1] ); }
      else
      {
    ...
    else if ( nlhs == 2 )
    {
      real_type * pX = Utils::mex_create_matrix_value( arg_out_0, 1, size );
      real_type * pY = Utils::mex_create_matrix_value( arg_out_1, 1, size );
      if ( ISO ) { LOOPXY2 ptr->eval_ISO_DDD( *s, *t, *pX, *pY ); }
      else
      {
...
static void do_evaluate( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
#define LOOPXY1 for ( mwSize i = 0; i < npts; ++i, s += incs, t += inct, pXY += 4 )
#define LOOPXY2 for ( mwSize i = 0; i < npts; ++i, s += incs, t += inct, ++pTH, ++pK, ++pX, ++pY )

    if ( nlhs == 1 )
    {
      real_type * pXY = Utils::mex_create_matrix_value( arg_out_0, 4, size );
      if ( ISO ) { LOOPXY1 ptr->evaluate_ISO( *s, *t, pXY[0], pXY[1], pXY[2], pXY[3] ); }
      else
      {
    ...
    else if ( nlhs == 4 )
    {
      ...
      if ( ISO ) { LOOPXY2 ptr->evaluate_ISO( *s, *t, *pTH, *pK, *pX, *pY ); }
      else
      {
...
#define LOOPXY1 for ( mwSize i = 0; i < npts; ++i, ++s, pXY += 4 )
#define LOOPXY2 for ( mwSize i = 0; i < npts; ++i, ++s, ++pTH, ++pK, ++pX, ++pY )
...
static void do_theta( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_theta_D( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_theta_DD( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
#define CMD CMD_BASE "('theta_DD',OBJ,s): "
  ...
  for ( mwSize i = 0; i < size; ++i ) *theta++ = ptr->theta_DD( *s++ );
...
static void do_theta_DDD( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_kappa( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_kappa_D( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_kappa_DD( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
#define CMD CMD_BASE "('kappa_DD',OBJ,s): "
  ...
  for ( mwSize i = 0; i < size; ++i ) *kappa_DD++ = ptr->kappa_DD( *s++ );
...
static void do_xy_begin( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_theta_begin( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_kappa_begin( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_xy_end( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_theta_end( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
static void do_kappa_end( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
{
  G2LIB_CLASS * ptr = Utils::mex_convert_mx_to_ptr<G2LIB_CLASS>( arg_in_1 );
...
#define CMD_MAP_FUN                                                                                               \
  { "length", do_length }, { "delete", do_delete }, { "make_a_copy", do_make_a_copy }, { "build2", do_build2 },   \
    { "bbox", do_bbox }, { "bbTriangles", do_bbTriangles }, { "change_origin", do_change_origin },                \
    { "translate", do_translate }, { "rotate", do_rotate }, { "scale", do_scale }, { "reverse", do_reverse },     \
    { "trim", do_trim }, { "distance", do_distance }, { "closest_point", do_closest_point },                      \
    { "collision", do_collision }, { "intersect", do_intersect }, { "findST", do_findST }, { "info", do_info },   \
    { "evaluate", do_evaluate }, { "eval", do_eval }, { "eval_D", do_eval_D }, { "eval_DD", do_eval_DD },         \
    { "eval_DDD", do_eval_DDD }, { "theta", do_theta }, { "theta_D", do_theta_D }, { "theta_DD", do_theta_DD },   \
    { "theta_DDD", do_theta_DDD }, { "kappa", do_kappa }, { "kappa_D", do_kappa_D }, { "kappa_DD", do_kappa_DD }, \
    { "xy_begin", do_xy_begin }, { "x_begin", do_x_begin }, { "y_begin", do_y_begin },                            \
