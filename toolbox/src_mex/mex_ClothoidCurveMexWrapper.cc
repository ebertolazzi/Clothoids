...
#define MEX_ERROR_MESSAGE                                                                   \
  ...
  "\n" MEX_INFO_MESSAGE( "ClothoidCurveMexWrapper" ) MEX_INFO_MESSAGE_END
...
#undef CMD
  ...
  static void do_infinity( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
  {
...
#define CMD "ClothoidCurveMexWrapper('distanceBySample',OBJ,x,y,ds): "
    ...
    if ( nlhs > 0 )
    {
      ...
      if ( nlhs > 1 )
      {
        ...
        for ( mwSize i = 0; i < size; ++i ) *dst++ = ptr->distance_by_sample( ds, *x++, *y++, *s++ );
      }
      else
      {
        for ( mwSize i = 0; i < size; ++i ) *dst++ = ptr->distance_by_sample( ds, *x++, *y++ );
      }
    }
...
#define CMD "ClothoidCurveMexWrapper('closest_point_by_sample',OBJ,x,y,ds): "
    ...
    mwSize size = nrx * ncx;
    for ( mwSize i = 0; i < size; ++i ) *dst++ = ptr->closest_point_by_sample( ds, *x++, *y++, *X++, *Y++, *S++ );
...
#undef CMD
  ...
  static void do_optimized_sample( int nlhs, mxArray * plhs[], int nrhs, mxArray const * prhs[] )
  {
#define CMD "ClothoidCurveMexWrapper('optimized_sample',OBJ,npts,max_angle,offs[,'ISO'/'SAE']): "
    ...
    if ( ISO )
      ptr->optimized_sample_ISO( offs, npts, max_angle, s );
    else
      ptr->optimized_sample_SAE( offs, npts, max_angle, s );
