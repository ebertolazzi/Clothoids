  ...
  const G2lib::real_type            curve_separation_tol = 100.0 )
{
  ...
  for ( size_t i = 0; i < Curves.size() - 1; ++i )
  {
    ...
    for ( const auto & peak_loc : Curves[i + 1].s_peaks ) { Waylines.emplace_back( wayLine{ peak_loc, -4.0, 4.0 } ); }
...
G2lib::real_type compute_curvature_cost(
  ...
  std::vector<G2lib::real_type> & offsets )
{
  ...
  for ( G2lib::integer i = 0; i < n_waylines; ++i ) { curvature_cost += KK_new[i] * KK_new[i]; }
...
int main()
{
  ...
  real_type Y[]{ 0,
                 ...
                 8.40520055326609,
                 3.73571064876812,
                 5.75374942922424e-05 };

  std::vector<curveLocProp> Curves{ { 1, 59.49, 145.5, { 85.12, 120.18 } },
  ...
  // create a vector of wayline using curve locations and peaks positions. if distance between end of one curve and
  // start of next curve is more than curve_separation_tol, then add one or more wayline in the middle depending on the
  // distance
