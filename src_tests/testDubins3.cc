...
int main()
{
  ...
  for ( size_t i{ 0 }; i < ilist.size(); ++i ) fmt::print( "s1[ {} ] = {}\n", i, ilist[i].first );

  for ( size_t i{ 0 }; i < ilist.size(); ++i ) fmt::print( "s2[ {} ] = {}\n", i, ilist[i].second );
  ...
  for ( size_t i{ 0 }; i < ilist.size(); ++i )
    fmt::print(
      ...
      DB1.Y( ilist[i].first ),
      i,
      DB2.X( ilist[i].second ),
      i,
      DB2.Y( ilist[i].second ) );
