...
namespace G2lib
{
  ...
  public:
    ...
    real_type X_D( real_type const s ) const { return cos( theta( s ) ); }
    real_type Y_D( real_type const s ) const { return sin( theta( s ) ); }
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    real_type X_ISO( real_type const s, real_type const offs ) const { return X( s ) + offs * nor_x_ISO( s ); }
    real_type Y_ISO( real_type const s, real_type const offs ) const { return Y( s ) + offs * nor_y_ISO( s ); }
    real_type X_ISO_D( real_type const s, real_type const offs ) const { return X_D( s ) + offs * nor_x_ISO_D( s ); }
    real_type Y_ISO_D( real_type const s, real_type const offs ) const { return Y_D( s ) + offs * nor_y_ISO_D( s ); }
    ...
    real_type X_SAE( real_type const s, real_type const offs ) const { return X( s ) + offs * nor_x_SAE( s ); }
    real_type Y_SAE( real_type const s, real_type const offs ) const { return Y( s ) + offs * nor_y_SAE( s ); }
    real_type X_SAE_D( real_type const s, real_type const offs ) const { return X_D( s ) + offs * nor_x_SAE_D( s ); }
    real_type Y_SAE_D( real_type const s, real_type const offs ) const { return Y_D( s ) + offs * nor_y_SAE_D( s ); }
    ...
    real_type tg_x_D( real_type const s ) const { return -sin( theta( s ) ) * theta_D( s ); }
    real_type tg_y_D( real_type const s ) const { return cos( theta( s ) ) * theta_D( s ); }
    ...
    [[nodiscard]] autodiff::dual1st tg_y( autodiff::dual1st const & s ) const;
    [[nodiscard]] autodiff::dual2nd tg_y( autodiff::dual2nd const & s ) const;

    [[nodiscard]] autodiff::dual1st nor_x_ISO( autodiff::dual1st const & s ) const;
    [[nodiscard]] autodiff::dual2nd nor_x_ISO( autodiff::dual2nd const & s ) const;

    [[nodiscard]] autodiff::dual1st nor_y_ISO( autodiff::dual1st const & s ) const;
    [[nodiscard]] autodiff::dual2nd nor_y_ISO( autodiff::dual2nd const & s ) const;

    [[nodiscard]] autodiff::dual1st nor_x_SAE( autodiff::dual1st const & s ) const { return -nor_x_ISO( s ); }
    [[nodiscard]] autodiff::dual2nd nor_x_SAE( autodiff::dual2nd const & s ) const { return -nor_x_ISO( s ); }

    [[nodiscard]] autodiff::dual1st nor_y_SAE( autodiff::dual1st const & s ) const { return -nor_y_ISO( s ); }
    [[nodiscard]] autodiff::dual2nd nor_y_SAE( autodiff::dual2nd const & s ) const { return -nor_y_ISO( s ); }
#endif
  };
