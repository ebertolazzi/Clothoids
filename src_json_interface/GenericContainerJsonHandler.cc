...
static
bool
isStringAnImaginaryNumber( string const & str_input,
...
static
bool
isStringAComplexNumber(string const & str_input, complex_type & out_number, string const & im_unit) {
  ...
  if (real_imag.size() !=2 ) return false;
  double real, imag;
  if ( (isStringARealNumber(real_imag[0], real)) &&
       (isStringAnImaginaryNumber(real_imag[1], imag, im_unit)) ) {
    ...
    case GC_LONG:
      {
        ...
        if ((type2 == GC_REAL) ||
            (type2 == GC_COMPLEX) ||
            (type2 == GC_STRING) ) return type2;
      }
      break;
    case GC_REAL:
      {
        if ( (type2 == GC_INTEGER) || (type2 == GC_LONG) ) return GC_REAL;
        if ( (type2 == GC_COMPLEX) || (type2 == GC_STRING) ) return type2;
      }
      break;
    case GC_COMPLEX:
      {
    ...
    case GC_MAT_LONG:
      {
        ...
        if ((type2 == GC_MAT_REAL) || (type2 == GC_MAT_COMPLEX) )return type2;
      }
      break;
    case GC_MAT_REAL:
      {
        if ( (type2 == GC_MAT_INTEGER) || (type2 == GC_MAT_LONG) ) return GC_MAT_REAL;
...
static
void
convertGenericVectorToStringVector(GenericContainer & root, string const & im_unit) {
  vector<string> new_vec;

  for ( GenericContainer const & gc : root.get_vector() ) {
    stringstream ss;
    if ( gc.get_type() == GC_STRING )       ss << gc.get_string();
    else if ( gc.get_type() == GC_INTEGER ) ss << gc.get_int();
    else if ( gc.get_type() == GC_LONG )    ss << gc.get_long();
    else if ( gc.get_type() == GC_REAL )    real_to_stream(gc.get_real(), ss);
    else if ( gc.get_type() == GC_COMPLEX ) complex_to_stream(gc.get_complex(), ss, im_unit);
...
convertGenericVectorToLongVector(GenericContainer & root) {
  ...
  for ( GenericContainer & gc : root.get_vector() ) {
    if ( gc.get_type() != GC_LONG ) gc.promote_to_long();
...
convertGenericVectorToRealVector(GenericContainer & root) {
  ...
  for ( GenericContainer & gc : root.get_vector() ) {
    if ( gc.get_type() != GC_REAL ) gc.promote_to_real();
...
convertGenericVectorToComplexVector(GenericContainer & root) {
  ...
  for ( GenericContainer & gc : root.get_vector() ) {
    if ( gc.get_type() != GC_COMPLEX ) gc.promote_to_complex();
...
static
void convertGenericVectorToIntMat(GenericContainer & root, GCJsonMatrixOrder order) {
...
static
void
convertGenericVectorToLongMat(GenericContainer & root, GCJsonMatrixOrder order) {
  if ( root.get_vector().size() == 0 ) return;

  //first check if they have the same size
  unsigned num_rows = unsigned(root.get_vector()[0].get_vec_long().size());
  for ( GenericContainer & gc : root.get_vector() ) {
...
static
void
convertGenericVectorToRealMat(GenericContainer & root, GCJsonMatrixOrder order) {
  if ( root.get_vector().size() == 0 ) return;

  //first check if they have the same size
  unsigned num_rows = unsigned(root.get_vector()[0].get_vec_real().size());
  for ( GenericContainer & gc : root.get_vector() ) {
    if (gc.get_type() != GC_VEC_REAL) gc.promote_to_vec_real();
...
static
void
convertGenericVectorToComplexMat(GenericContainer & root, GCJsonMatrixOrder order) {
  if ( root.get_vector().size() == 0 ) return;

  //first check if they have the same size
  unsigned num_rows = unsigned(root.get_vector()[0].get_vec_complex().size());
  for ( GenericContainer & gc : root.get_vector() ) {
...
bool
GenericContainerJsonHandler::String(const char * str, SizeType length, bool copy) {
  ASSERT_STACK_SIZE
...
bool
GenericContainerJsonHandler::Key(const char * str, SizeType length, bool copy) {
