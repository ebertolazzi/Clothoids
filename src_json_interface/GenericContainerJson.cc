...
namespace GenericContainerNamespace {
  ...
  complex_to_stream( complex_type const & number,
    ...
    if ( number == complex_type(0,0) ) { out << "0" ; return; }
    if ( number.real() != 0 ) real_to_stream(number.real(), out);
    if ( number.imag() == 0 ) return;
    if ( number.real() != 0 ) out << "+";
  ...
  gc_to_writer( GenericContainer const & gc,
    ...
    case GC_VEC_BOOL:
      {
        writer.StartArray();
        for ( bool value : gc.get_vec_bool() ) writer.Bool( value );
        writer.EndArray();
      }
      break;
    case GC_VEC_INTEGER:
      {
        writer.StartArray();
        for ( int_type value : gc.get_vec_int() ) writer.Int( value );
        writer.EndArray();
      }
      break;
    case GC_VEC_LONG:
      {
        writer.StartArray();
        for ( long_type value : gc.get_vec_long() ) writer.Int64( value );
        writer.EndArray();
      }
      break;
    case GC_VEC_REAL:
      {
        writer.StartArray();
        for ( real_type value : gc.get_vec_real() ) writer.Double( value );
        writer.EndArray();
      }
      break;
    case GC_VEC_STRING:
