...
namespace G2lib
{
  namespace Interpolation
  {
    ...
    {
      if ( m_solver.spline().constraints( theta.data(), constraints_value.data() ) ) return 0;
      return 1;
    }
    ...
    {
      if ( !( m_solver.spline().jacobian( theta.data(), &m_jacobian_result.front() ) ) ) return 1;
    ...
    bool LMSolver::solve()
    {
      ...
      switch ( info )
      {
        case ( Eigen::Success ): return true; break;
        case ( Eigen::NumericalIssue ): throw std::runtime_error( "LM SOLVER: Numerical Issues" ); break;
        case ( Eigen::NoConvergence ): throw std::runtime_error( "LM SOLVER: No Convergence" ); break;
        case ( Eigen::InvalidInput ): throw std::runtime_error( "LM SOLVER: Invalid Input" ); break;
      }
      return false;
    }
