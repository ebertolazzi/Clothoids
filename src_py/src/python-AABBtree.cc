  ...
  namespace python
  {
    void wrap_AABBtree( py::module & m )
    {
      ...
      py::class_<BBox, PyPtrBBox>(
        ...
        .def(
          py::init<real_type, real_type, real_type, real_type, integer, integer>(),
          py::arg( "xmin" ),
          py::arg( "ymin" ),
          py::arg( "xmax" ),
          py::arg( "ymax" ),
        ...
        .def(
          py::init(
            []( PyVecPtrBBox bboxes, integer id, integer ipos )
            {
              std::vector<BBox::PtrBBox> _bboxes;
              for ( auto & el : bboxes ) { _bboxes.push_back( static_cast<BBox::PtrBBox>( el ) ); }
        ...
        .def(
          py::init(
            [](
              std::tuple<std::tuple<real_type, real_type>, std::tuple<real_type, real_type>> extrema,
              integer                                                                        id,
              integer                                                                        ipos )
            {
        ...
        .def(
          "join",
          []( PyPtrBBox self, PyVecPtrBBox bboxes )
          {
            std::vector<BBox::PtrBBox> _bboxes;
            for ( auto & el : bboxes ) { _bboxes.push_back( static_cast<BBox::PtrBBox>( el ) ); }
            self->join( _bboxes );
      ...
      py::class_<AABBtree>(
        ...
        .def(
          "build",
          []( AABBtree & self, PyVecPtrBBox bboxes )
          {
            std::vector<BBox::PtrBBox> _bboxes;
            for ( auto & el : bboxes ) { _bboxes.push_back( static_cast<BBox::PtrBBox>( el ) ); }
            self.build( _bboxes );
        ...
        .def(
          "intersect",
          []( AABBtree & self, AABBtree const & tree, bool swap_tree )
          {
            ...
            for ( auto & el : intersectionList )
            {
              ...
              PyPtrBBox     _left = std::make_shared<BBox>(
                left->Xmin(),
                left->Ymin(),
                left->Xmax(),
                left->Ymax(),
                left->Id(),
                left->Ipos() );
              PyPtrBBox _right = std::make_shared<BBox>(
                right->Xmin(),
                right->Ymin(),
                right->Xmax(),
                right->Ymax(),
        ...
        .def(
          "min_distance",
          []( AABBtree & self, real_type x, real_type y )
          {
            ...
            {
              result.push_back(
                std::make_shared<BBox>( el->Xmin(), el->Ymin(), el->Xmax(), el->Ymax(), el->Id(), el->Ipos() ) );
            }
            return result;
          },
          py::arg( "x" ),
        ...
        .def(
          "print",
          []( const AABBtree & self, integer level = 0 )
          {
            std::ostringstream str;
            self.print( str, 0 );
            return str.str();
