...
namespace G2lib
{
  namespace Interpolation
  {
    ...
    {
      if ( theta_size != m_solver.theta_size() ) return false;

      if ( constraints_size != m_solver.constraints_size() ) return false;

      if (
        ( theta_min == nullptr ) || ( theta_max == nullptr ) || ( constraints_min == nullptr ) ||
      ...
      Number * lambda )
    {
      ...
      if ( init_theta ) { std::copy( m_solver.theta_solution().begin(), m_solver.theta_solution().end(), theta_ics ); }
      return true;
    }
      ...
      Number *       jacobian_values )
    {
      if ( theta_size != m_solver.theta_size() ) return false;
      if ( constraints_size != m_solver.constraints_size() ) return false;
      if ( jacobian_pattern_size != m_solver.jacobian_pattern_size() ) return false;
      ...
      if ( jacobian_values != NULL ) { jac_eval_ok = m_solver.spline().jacobian( theta, jacobian_values ); }
    ...
    bool IpoptSolver::solve()
    {
      ...
      if ( status != ApplicationReturnStatus::Solve_Succeeded ) return false;

      status = app->OptimizeTNLP( spline_problem );
      switch ( status )
      {
        case ( ApplicationReturnStatus::Solve_Succeeded ): return true; break;
        case ( ApplicationReturnStatus::Solved_To_Acceptable_Level ): return true; break;
        ...
        case ( ApplicationReturnStatus::Invalid_Problem_Definition ):
          ...
          break;
        case ( ApplicationReturnStatus::Invalid_Option ): std::runtime_error( "IPOPT Error :: Invalid_Option" ); break;
        ...
        case ( ApplicationReturnStatus::Insufficient_Memory ):
          ...
          break;
        case ( ApplicationReturnStatus::Internal_Error ): std::runtime_error( "IPOPT Error :: Internal_Error" ); break;
      }
      return false;
    }
