...
namespace G2lib
{
  namespace Interpolation
  {
    ...
    void Interpolator::check_input()
    {
      ...
      if ( xs().size() != ys().size() ) { throw std::runtime_error( "Input vectors must be of same length" ); }

      /* Check enough input */
      if ( xs().size() < 2 ) { throw std::runtime_error( "Input size too small" ); }
      ...
      if ( *min_chk == 0 ) { throw std::runtime_error( "Minimal distance too short" ); }
      if ( *min_chk < ( 1e-10 * *max_chk ) ) { throw std::runtime_error( "Problem with too much deviation" ); }
    }

    void Interpolator::build_clothoid_list( const std::vector<real_type> & theta, ClothoidList & result )
    {
      if ( theta.size() < 2 ) { throw std::runtime_error( "Result has only two values??" ); }
      result.init();
    ...
    void Interpolator::build_using_lm_solver( ClothoidList & result )
    {
      ...
      solver.guess();
      if ( !solver.solve() ) { throw std::runtime_error( "Optimization failed - Unknown reason" ); }
