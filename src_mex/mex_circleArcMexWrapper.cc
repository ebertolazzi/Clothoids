...
DATA_NEW( mxArray * & mx_id ) {
  Circle::CircleArc * ptr = new Circle::CircleArc();
  mx_id = convertPtr2Mat<Circle::CircleArc>(ptr);
  return ptr ;
}
...
DATA_GET( mxArray const * & mx_id ) {
  return convertMat2Ptr<Circle::CircleArc>(mx_id);
}

static
void
DATA_DELETE( mxArray const * & mx_id ) {
  Circle::CircleArc * ptr = convertMat2Ptr<Circle::CircleArc>(mx_id);
  delete ptr ;
}
...
mexFunction( int nlhs, mxArray       *plhs[],
  ...
  try {
    ...
    bool do_new = cmd == "new" ;
    Circle::CircleArc * ptr = do_new ? DATA_NEW(arg_out_0) : DATA_GET(arg_in_1);
    ...
    if ( do_new || cmd == "build" ) {
      ...
      if ( nrhs >= 6+kk && nrhs <= 7+kk ) {
        ...
        Circle::valueType const * p0 = getVectorPointer( prhs[1+kk], size0, "CircleArc: `p0` expected to be a real vector" );
        Circle::valueType const * p1 = getVectorPointer( prhs[2+kk], size1, "CircleArc: `p1` expected to be a real vector" );
        Circle::valueType const * p2 = getVectorPointer( prhs[3+kk], size2, "CircleArc: `p2` expected to be a real vector" );
